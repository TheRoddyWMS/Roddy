...
//        }
    ...
    static void printRuntimeConfiguration(CommandLineCall commandLineCall) {
        ...
        if (commandLineCall.isOptionSet(RoddyStartupOptions.showentrysources)) {
                    ...
                    cvalues.allValues.each {
                        String id, ConfigurationValue cvalue ->
                            ...
                            for (ConfigurationValue iValue : inheritanceList.reverse()) {
                                println(((first ? '    ' : '  * ') + '') +
                                                iValue.value.padRight(70).substring(0, 70) +
                                                ' ' +
                                                iValue?.configuration?.preloadedConfiguration?.file)
                                first = false
                            }
                    }
                }
                    ...
                    cvalues.allValues.each {
                        String id, ConfigurationValue cvalue ->
                            try {
                                ...
                                println(id.padRight(50).substring(0, 50) + " " +
                                                cvalue.value.padRight(70).substring(0, 70) + " " +
                                                path)
                            } catch (Exception ex) {
                                println(ex)
                            }
            ...
            for (executionContext in executionContexts) {
                System.out.println(ConfigurationConverter.
                        convertAutomatically(executionContext, executionContext.analysis.configuration))
            }
        }
    }
