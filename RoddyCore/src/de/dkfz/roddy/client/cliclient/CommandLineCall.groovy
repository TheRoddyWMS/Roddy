...
class CommandLineCall {
    ...
    CommandLineCall(List<String> args) {
        ...
        for (String optArg in options) {
            ...
            try {
                ...
                List<String> values = option.acceptsParameters ? split[1].split(SPLIT_COMMA)?.toList() : null
    ...
    String getOptionValue(RoddyStartupOptions option) {
        optionsMap.get(option)?.first()
    }

    List<String> getOptionList(RoddyStartupOptions option) {
     ...
     * @return
     */
    List<ConfigurationValue> getSetConfigurationValues() {
        ...
        for (eVal in externalConfigurationValues) {
            ...
            // Surround value with single quotes '' to prevent string interpretation for e.g. execution in bash
