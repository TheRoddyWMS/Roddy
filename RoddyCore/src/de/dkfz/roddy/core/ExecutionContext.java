...
public class ExecutionContext implements JobStatusListener {
    ...
    public LockFileGroup createLockFiles(FileGroup fileGroup) {
        return createLockFiles(fileGroup.getFilesInGroup());
    }
...
//        StreamingBufferFileGroup
    ...
    public File getFileForAnalysisToolsArchiveOverview() {
        return new File(getCommonExecutionDirectory() + FileSystemInfoProvider.getInstance().getPathSeparator() + RuntimeService.FILENAME_ANALYSES_MD5_OVERVIEW);
    }
    ...
    public Map<String, Integer> readJobStateLogFile() {
