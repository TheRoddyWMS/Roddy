...
// If the file is not valid then also temporary parent files should be invalidated! Or at least checked.
    ...
    public TextFile createInitialTextFile(ExecutionContext ec) {
        ...
        tf.setAsSourceFile();
        if (!FileSystemInfoProvider.getInstance().checkFile(tf.getPath()))
            FileSystemInfoProvider.getInstance().createFileWithDefaultAccessRights(true, tf.getPath(), ec, true);
        return tf;
    }
}
