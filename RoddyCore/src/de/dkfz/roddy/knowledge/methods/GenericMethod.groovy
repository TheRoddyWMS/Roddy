 ...
 * and used for the further process.
 */
@CompileStatic
class GenericMethod {
    ...
    static <F extends FileObject> F callGenericTool(String toolName,
        ...
        F result = new GenericMethod(
                ...
                input,
                null,
                additionalInput)._callGenericToolOrToolArray()
        return result
    }
    ...
    static <F extends FileGroup> F callGenericToolWithFileGroupOutput(String toolName,
        ...
        F result = new GenericMethod(
                ...
                input,
                indices,
                additionalInput)._callGenericToolOrToolArray() as F
        return result
    }
    ...
    /**
     * This field is only set if you need index values for output file group objects
     * (specifically said: String indices!)
     */
    ...
    /**
     * A combined list of all input values. This list also contains entries from e.g. the configuration and,
     * e.g., from the command factory.
     */
    ...
    GenericMethod(String toolName,
        ...
        if (null != outputFileGroupIndices) {
            Preconditions.checkArgument(
                    outputFileGroupIndices.toList().unique().size() == outputFileGroupIndices.toList().size())
        ...
        if (inputObject instanceof FileGroup) {
            // Might be null at some point... Should we throw something?
            this.firstInputFile = (inputObject as FileGroup).getFilesInGroup().get(0)
        } else if (inputObject instanceof BaseFile) {
            this.firstInputFile = inputObject as BaseFile
        } else {
            throw new RuntimeException("GenericMethod called with unknown instance type: ${inputObject}")
        }
    ...
    public <F extends FileObject> F _callGenericToolOrToolArray() {
        ...
        if (!calledTool.isToolGeneric()) {
            throw new ConfigurationError("Non-generic tool called via the generic call method: " + toolName +
                    ". Check your configuration files. Generic tools have input or output parameters " +
                    "or use 'overrideresourcesets'.", 
                    "")
        }
        ...
        for (int i = 0; i < calledTool.getInputParameters(context).size(); i++) {
            ...
            if (toolParameter instanceof ToolFileParameter) {
                ...
                if(allInputValues.size() <= i ) {
                    throw new ConfigurationError("Not all input parameters were set for ${calledTool.id}. " +
                            "Expected ${calledTool.getInputParameters(context).size()}. Breaking loop and trying to" +
                            "go on.", "GenericMethod")
                }
                if(allInputValues[i] == null){
                    logger.severe("There is an input mismatch for the tool ${calledTool.id}." +
                            "Expected ${_tp.fileClass} but got null. Trying to go on.")
                    continue
                }
                if (!allInputValues[i].class == _tp.fileClass){
                    throw new ConfigurationError("Class mismatch for ${allInputValues[i]} should be of " +
                            "'class ${_tp.fileClass}.", "GenericMethod")
                }
        ...
        for (ToolFileParameter childFileParameter in (fileParameter.getFiles() as List<ToolFileParameter>)) {
            try {
                ...
                try {
                    String setterMethod =
                            "set" +
                            childFileParameter.parentVariable[0].toUpperCase() +
                            childFileParameter.parentVariable[1..-1]
                ...
                if (_field == null && _method == null) {
                    try {
                        context.addErrorEntry(ExecutionContextError.EXECUTION_FILECREATION_FIELDINACCESSIBLE.
                                expand("tool $toolName, " +
                                        "class ${bf.getClass().getName()}, " +
                                        "field ${childFileParameter.parentVariable}"))
                    } catch (Exception ex) {
                    }
                    continue
                }
    ...
    FileObject createOutputFileGroup(ToolFileGroupParameter tfg) {
        ...
        if (tfg.files) {
            ...
            /**
             * This can only be the case if files is set. Otherwise we need a different way to identify files.
             * E.g. based on index values... How do we set them?
             */
            ...
            if (!outputFileGroupIndices) {
                throw new RuntimeException(
                        "A tool which outputs a filegroup with index values needs to be called properly! " +
                                "Pass index values in the call.")
            }
            ToolFileParameter autoToolFileParameter =
                    new ToolFileParameter(tfg.genericFileClass,
                            [],
                            tfg.scriptParameterName,
     ...
     *
     * @param fileParameter
     * @param fileGroupIndex May be null or empty! Is set, when a filegroup and its children are created.
     *        Holds the index value within the filegroup (numeric or a string)
